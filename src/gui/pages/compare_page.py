...
import time
from PySide6.QtWidgets import QWidget, QVBoxLayout, QLabel, QPushButton, QFrame, QGridLayout, QHBoxLayout, QSizePolicy, QScrollArea
from PySide6.QtCore import Qt
from PySide6.QtWidgets import QGraphicsDropShadowEffect
from PySide6.QtCharts import QChart, QChartView, QBarSeries, QBarSet, QLineSeries, QPieSeries, QValueAxis, QCategoryAxis
...
class ComparePage(QWidget):
    def __init__(self, router):
        ...
        self.card_line, self.card_line_layout, self.desc_line = make_card(
            ...
            "Le ratio (%) = taille compressée / taille originale. Plus il est faible, mieux c'est."
        )
        # --- Carte 3: Time chart ---
        ...
        self.chart_line_view = QChartView()
        self.chart_time_view = QChartView()
        self.chart_donut_view = QChartView()

        for view in (self.chart_bar_view, self.chart_line_view, self.chart_time_view, self.chart_donut_view):
            view.setRenderHint(QPainter.Antialiasing)
            view.setStyleSheet("background: transparent;")
            view.setMinimumHeight(240)
            view.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)

        self.chart_donut_view.setMinimumHeight(260)
    ...
    def generate_from_file(self, use_last_only: bool = False):
        ...
        # Huffman
        if huf_path.exists():
            size_huffman = huf_path.stat().st_size
            time_huffman = last.get("times", {}).get("Huffman") if last else None
            if time_huffman is None:
                start = time.perf_counter()
                compressed_huffman = huffman_compress(data)
                time_huffman = time.perf_counter() - start
            else:
                compressed_huffman = None
        else:
            start = time.perf_counter()
            compressed_huffman = huffman_compress(data)
            time_huffman = time.perf_counter() - start
            huf_path.write_bytes(compressed_huffman)
            size_huffman = len(compressed_huffman)

        # LZW
        if lzw_path.exists():
            size_lzw = lzw_path.stat().st_size
            time_lzw = last.get("times", {}).get("LZW") if last else None
            if time_lzw is None:
                start = time.perf_counter()
                compressed_lzw = lzw_encoder(data)
                time_lzw = time.perf_counter() - start
            else:
                compressed_lzw = None
        else:
            start = time.perf_counter()
            compressed_lzw = lzw_encoder(data)
            time_lzw = time.perf_counter() - start
            lzw_path.write_bytes(compressed_lzw)
