...
function MessageBubble({
  ...
  return (
    ...
    >
      ...
      {/* Bubble */}
      <div
        className={`max-w-[78%] rounded-3xl px-4 py-3 text-sm leading-relaxed shadow-lg whitespace-pre-wrap
          ${
            isUser
...
export default function AssistantPage() {
  ...
  return (
    <div className="flex h-[calc(100dvh-0px)] flex-col md:h-screen text-white">
      {/* ── HEADER ── */}
      <div className="shrink-0 border-b border-white/10 bg-white/5 px-5 py-4 backdrop-blur-md sm:px-8">
        <motion.div
        ...
        >
          ...
          <div>
            <h1 className="text-lg font-bold sm:text-xl">CompressBot</h1>
            ...
            <div className="flex flex-wrap gap-2">
              {SUGGESTIONS.map((s) => (
                <button
                  ...
                  className="rounded-full border border-white/20 bg-white/5 px-3 py-1.5 text-xs text-white/70 transition hover:bg-white/15 hover:text-white"
                >
                  {s}
                </button>
              ))}
      ...
      {/* ── INPUT ── */}
      <div className="shrink-0 border-t border-white/10 bg-white/5 px-4 py-4 backdrop-blur-md sm:px-8">
        <div className="mx-auto flex max-w-2xl items-end gap-3">
          <textarea
            ...
            className="flex-1 resize-none rounded-2xl border border-white/20 bg-white/10 px-4 py-3 text-sm text-white placeholder-white/40 backdrop-blur-md transition focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-400"
          ...
          <button
            ...
            className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500 to-purple-600 shadow-lg transition hover:scale-105 disabled:cursor-not-allowed disabled:opacity-40"
            aria-label="Envoyer"
          >
            <Send size={18} />
          </button>
        </div>
        <p className="mx-auto mt-2 max-w-2xl text-center text-[10px] text-white/25">
